    ...
    socket.on('join_game', async ({ gameId, playerId }) => {
      try {
        ...
        const game = await prisma.game.findUnique({
          where: { id: gameId },
          include: {
            whitePlayer: { select: { id: true, name: true, rank: true, currentPoints: true } },
            blackPlayer: { select: { id: true, name: true, rank: true, currentPoints: true } },
          },
        })
        ...
        socket.emit('game_state', {
          ...
          result: gs.result,
          yourColor: color,
          whitePlayer: { id: game.whitePlayer.id, name: game.whitePlayer.name, rank: game.whitePlayer.rank },
          blackPlayer: { id: game.blackPlayer.id, name: game.blackPlayer.name, rank: game.blackPlayer.rank },
          whiteTimeMs,
          blackTimeMs,
